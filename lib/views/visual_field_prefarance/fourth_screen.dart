...
class FourthScreen extends StatefulWidget {
  final String userId;
  const FourthScreen({super.key, required this.userId});
...
class _FourthScreenState extends State<FourthScreen> {
  ...
  Future<void> _sendDataToFirestore(int score, int reactionTime) async {
    try {
      ...
      await firestore.collection('visual_field_preference').doc(widget.userId).collection('4th_screen').doc(widget.userId).set({
        'score': score,
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => FifthScreen(userId: userId)),
              );
            },
