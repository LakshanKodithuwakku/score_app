...
import 'package:score_app/views/visual_field_prefarance/sixth_screen.dart';
import 'dart:async';
...
class FifthScreen extends StatefulWidget {
  final String userId;
  const FifthScreen({super.key, required this.userId});
...
class _FifthScreenState extends State<FifthScreen> {
  ...
  Future<void> _sendDataToFirestore(int score, int reactionTime) async {
    try {
      ...
      await firestore.collection('visual_field_preference').doc(widget.userId).collection('5th_screen').doc(widget.userId).set({
        'score': score,
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => SixthScreen(userId: userId)),
              );
            },
