...
class Question4Screen extends StatefulWidget {
  final String userId;
  const Question4Screen({Key? key, required this.userId}) : super(key: key);
  @override
...
class _Question4ScreenState extends State<Question4Screen> {
  ...
  Future<void> _sendDataToFirestore(int marks) async {
    try {
      ...
      await firestore.collection('visual_field_preference').doc(widget.userId).collection('Q4_marks').doc(widget.userId).set({
        'Question 04': marks,
      });
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(
              'Does your child orient the head downwards when walking?',
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => Question5Screen(userId: userId)),
                );
              },
