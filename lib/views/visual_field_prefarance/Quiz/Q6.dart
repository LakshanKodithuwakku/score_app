...
class _Question6ScreenState extends State<Question6Screen> {
  ...
  Future<void> _sendDataToFirestore(int marks) async {
    try {
      ...
      await firestore.collection('visual_field_preference').doc(widget.userId).collection('Q6_marks').doc(widget.userId).set({
        'Question 06': marks,
      });
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(
              'Does your child pay attention only to objects Infront of him?',
