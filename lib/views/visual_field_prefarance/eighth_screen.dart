...
class EighthScreen extends StatefulWidget {
  ...
  const EighthScreen({super.key, required this.userId});
...
class _EighthScreenState extends State<EighthScreen> {
  ...
  Future<void> _sendDataToFirestore(int score, int reactionTime) async {
    try {
      ...
      await firestore.collection('visual_field_preference').doc(widget.userId).collection('8th_screen').doc(widget.userId).set({
        'score': score,
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => NinthScreen(userId: userId)),
              );
            },
