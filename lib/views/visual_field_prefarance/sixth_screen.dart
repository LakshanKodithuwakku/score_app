...
import 'package:score_app/views/visual_field_prefarance/seventh_screen.dart';
import 'dart:async';
...
class SixthScreen extends StatefulWidget {
  ...
  const SixthScreen({super.key, required this.userId});
  @override
...
class _SixthScreenState extends State<SixthScreen> {
  ...
  Future<void> _sendDataToFirestore(int score, int reactionTime) async {
    try {
      ...
      await firestore.collection('visual_field_preference').doc(widget.userId).collection('6th_screen').doc(widget.userId).set({
        'score': score,
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => SeventhScreen(userId: userId)),
              );
            },
