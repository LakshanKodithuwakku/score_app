...
class VC_Question2 extends StatefulWidget {
  final String userId;
  const VC_Question2({Key? key, required this.userId}) : super(key: key);
  @override
  _VC_Question2State createState() => _VC_Question2State();
}

class _VC_Question2State extends State<VC_Question2> {
  String selectedAnswer = '';

  Future<void> _sendDataToFirestore(int marks) async {
    try {
      FirebaseFirestore firestore = FirebaseFirestore.instance;
      await firestore.collection('visual_complexity').doc(widget.userId).collection('Q2_marks').doc(widget.userId).set({
        'Question 02': marks,
      });
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(
              'Does your child have difficulties with looking at objects?',
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => VC_Question3(userId: userId)),
                );
              },
