...
class VC_Question3 extends StatefulWidget {
  final String userId;
  const VC_Question3({Key? key, required this.userId}) : super(key: key);
  @override
  _VC_Question3State createState() => _VC_Question3State();
}

class _VC_Question3State extends State<VC_Question3> {
  String selectedAnswer = '';

  Future<void> _sendDataToFirestore(int marks) async {
    try {
      FirebaseFirestore firestore = FirebaseFirestore.instance;
      await firestore.collection('visual_complexity').doc(widget.userId).collection('Q3_marks').doc(widget.userId).set({
        'Question 03': marks,
      });
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(
              'Does your child have difficulties with looking at people?',
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => VC_Question4(userId: userId)),
                );
              },
