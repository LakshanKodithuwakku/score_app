...
class VC_Question8 extends StatefulWidget {
  final String userId;
  const VC_Question8({Key? key, required this.userId}) : super(key: key);
  @override
  _VC_Question8State createState() => _VC_Question8State();
}

class _VC_Question8State extends State<VC_Question8> {
  String selectedAnswer = '';

  Future<void> _sendDataToFirestore(int marks) async {
    try {
      FirebaseFirestore firestore = FirebaseFirestore.instance;
      await firestore.collection('visual_complexity').doc(widget.userId).collection('Q8_marks').doc(widget.userId).set({
        'Question 08': marks,
      });
      print('Question 08 mark added to Firestore: $marks');
    } catch (e) {
      print('Error adding Question 08 mark to Firestore: $e');
    }
  }
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(
              ' Can your child find a favorite toy easily when it is on a patterned surface?',
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => VC_EndScreen(userId: userId)),
                );
              },
