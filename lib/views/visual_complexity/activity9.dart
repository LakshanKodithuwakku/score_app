...
class Activity9 extends StatefulWidget {
  final String userId;
  const Activity9({Key? key, required this.userId}) : super(key: key);

  @override
  _Activity9State createState() => _Activity9State();
}
...
class _Activity9State extends State<Activity9> {
  ...
  Future<void> _sendDataToFirestore(int score, int reactionTime) async {
    try {
      ...
      await firestore.collection('visual_complexity').doc(widget.userId).collection('activity9').doc(widget.userId).set({
        'score': score,
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => Activity10(userId: userId)),
              );
            },
            child: Image.asset(
