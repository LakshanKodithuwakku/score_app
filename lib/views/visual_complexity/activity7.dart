...
class Activity7 extends StatefulWidget {
  final String userId;
  const Activity7({Key? key, required this.userId}) : super(key: key);

  @override
  _Activity7State createState() => _Activity7State();
}
...
class _Activity7State extends State<Activity7> {
  ...
  Future<void> _sendDataToFirestore(int score, int reactionTime) async {
    try {
      ...
      await firestore.collection('visual_complexity').doc(widget.userId).collection('activity7').doc(widget.userId).set({
        'score': score,
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) =>  Activity8(userId: userId)),
              );
            },
            child: Image.asset(
