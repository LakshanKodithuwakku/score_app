...
class Activity3 extends StatefulWidget {
  final String userId;
  const Activity3({Key? key, required this.userId}) : super(key: key);
...
class _Activity3State extends State<Activity3> {
  ...
  Future<void> _sendDataToFirestore(int score, int reactionTime) async {
    try {
      ...
      await firestore.collection('visual_complexity').doc(widget.userId).collection('activity3').doc(widget.userId).set({
        'score': score,
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => Activity4(userId: userId)),
              );
            },
            child: Image.asset(
          ...
          GestureDetector(
            ...
            child: Container(
              child: Center(
                child: Image.asset(
                  'assets/VC_images/fish_black.png',
                  height: 150.0,
                  width: 150.0,
                ),
              ),
