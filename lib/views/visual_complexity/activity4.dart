...
class Activity4 extends StatefulWidget {
  final String userId;
  const Activity4({Key? key, required this.userId}) : super(key: key);
...
class _Activity4State extends State<Activity4> {
  ...
  Future<void> _sendDataToFirestore(int score, int reactionTime) async {
    try {
      ...
      await firestore.collection('visual_complexity').doc(widget.userId).collection('activity4').doc(widget.userId).set({
        'score': score,
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) =>  Activity5(userId: userId)),
              );
            },
            child: Image.asset(
