...
class Activity8 extends StatefulWidget {
  final String userId;
  const Activity8({Key? key, required this.userId}) : super(key: key);

  @override
  _Activity8State createState() => _Activity8State();
}
...
class _Activity8State extends State<Activity8> {
  ...
  Future<void> _sendDataToFirestore(int score, int reactionTime) async {
    try {
      ...
      await firestore.collection('visual_complexity').doc(widget.userId).collection('activity8').doc(widget.userId).set({
        'score': score,
        ...
        children: [
          GestureDetector(
            onTap: () {
              ...
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => Activity9(userId: userId)),
              );
            },
            child: Image.asset(
              'assets/VC_images/complex_s_2.jpg',
              fit: BoxFit.cover,
          ...
          Row(
            ...
            children: [
              GestureDetector(
                ...
                child: Image.asset("assets/VC_images/teddy_c2.png",
                  height: 150.0,
                  width: 150.0,
                ),
