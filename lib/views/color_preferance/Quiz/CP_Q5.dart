...
class _Question5ScreenState extends State<Question5Screen> {
  ...
  Future<void> _sendDataToFirestore(int marks) async {
    try {
      ...
      await firestore.collection('color_preferance').doc(widget.userId).collection('Q5_marks').doc(widget.userId).set({
        'Question 05': marks,
      });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Color Preferance',
        ),
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(
              'When given colorful toys, does the child often choose one color over the others?',
