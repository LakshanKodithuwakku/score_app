...
class _Question2ScreenState extends State<Question2Screen> {
  ...
  Future<void> _sendDataToFirestore(int marks) async {
    try {
      ...
      await firestore.collection('color_preferance').doc(widget.userId).collection('Q2_marks').doc(widget.userId).set({
        'Question 02': marks,
      });
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(
              'Does the child often choose or look at brightly colored things? (e.g. lights or windows)',
