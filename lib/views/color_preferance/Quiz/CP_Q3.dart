...
class _Question3ScreenState extends State<Question3Screen> {
  ...
  Future<void> _sendDataToFirestore(int marks) async {
    try {
      ...
      await firestore.collection('color_preferance').doc(widget.userId).collection('Q3_marks').doc(widget.userId).set({
        'Question 03': marks,
      });
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(
              'Can find a favourite toy easily when it is amongst other toys ?',
