...
import 'package:score_app/views/moving_notmoving_detection/moving_notmoving_detection.dart';
...
class _EndScreenState extends State<EndScreen> {
  ...
  Future<void> _sendDataToFirestore(int totalMarks) async {
    try {
      ...
      await firestore.collection('color_preferance').doc(widget.userId).collection('total_marks').doc(widget.userId).set({
  ...
  @override
  Widget build(BuildContext context) {
    int TotalMarks = Provider.of<CP_MarksProvider>(context).totalMarks;
                ...
                style: const TextStyle(
                  ...
                  ElevatedButton(
                    onPressed:(){
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => StartScreenMoveOBJ()), //Activity0(userId: userId)), //navigate to next activity
                      );
                    } ,
