...
class _Question4ScreenState extends State<Question4Screen> {
  ...
  Future<void> _sendDataToFirestore(int marks) async {
    try {
      ...
      await firestore.collection('color_preferance').doc(widget.userId).collection('Q4_marks').doc(widget.userId).set({
        'Question 04': marks,
      });
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text(
              'Is the child drawn to colorful surroundings and spaces?',
